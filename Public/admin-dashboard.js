document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchAllTransactions() {
        ...
        try {
            const response = await fetch(`https://megalife-app-postgres.onrender.com/admin/transactions`, { headers: { 'Authorization': `Bearer ${token}`, 'Cache-Control': 'no-cache' } });
            if (!response.ok) throw new Error('Failed to fetch transactions');
            const transactions = await response.json();
            let tableHTML = `<div class="table-header"><span>User</span><span>Type</span><span>Details</span><span>Recipient</span><span>Amount</span><span>Date</span><span>Status</span><span>Actions</span></div><div class="table-body">`;
...
// Function to fetch and display ALL TRANSACTIONS (Final Corrected Version)
    // FINAL CORRECTED fetchAllTransactions function
    async function fetchAllTransactions() {
        const transTableContainer = document.querySelector('#allTransactionsContent .transactions-table');
        if (!transTableContainer) return;
        transTableContainer.innerHTML = '<div class="empty-state">Loading transactions...</div>';
        try {
            ...
            if (transactions.length > 0) {
                transactions.forEach(tx => {
                    ...
                    let actionButton = '';
                    // Only show the button if the transaction can be retried
                    if (tx.status === 'Processing' || tx.status === 'Failed') {
                    ...
                    tableHTML += `
                        <div class="table-row">
                            ...
                            <span class="actions">${actionButton}</span>
                        </div>
                    `;
                });
            } else {
